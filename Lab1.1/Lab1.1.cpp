...
glBufferSubData (GL_ARRAY_BUFFER, 0, numVertices*4*sizeof(GLfloat), vertices);
glBufferSubData (GL_ARRAY_BUFFER, numVertices*4*sizeof(GLfloat), numVertices*4*sizeof(GLfloat), colors);
return VBO;
}
...
glVertexAttribPointer(colorID, 4, GL_FLOAT, GL_FALSE, 0, BUFFER_OFFSET(numVertices*4*sizeof(GLfloat)));
}
